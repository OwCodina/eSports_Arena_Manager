...
public class EquipoController {
    ...
    public ResponseEntity<List<EquipoResponseDTO>> findAll(@RequestParam(required=false) String estado, @RequestParam(required=false) Long juegoId, @RequestParam(required=false) Long capitanId) {
        if (juegoId != null)
    ...
    public ResponseEntity<Map<String,Boolean>> estaActivo(@PathVariable Long id) {
    ...
    @PostMapping
    public ResponseEntity<EquipoResponseDTO> save(@Valid @RequestBody EquipoRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(equipoService.save(dto));
    }

    @PostMapping("/{id}/miembros")
    public ResponseEntity<MiembroEquipo> agregarMiembro(@PathVariable Long id, @Valid @RequestBody MiembroRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(equipoService.agregarMiembro(id, dto)); }

    @PutMapping("/{id}") public ResponseEntity<EquipoResponseDTO> update(@PathVariable Long id, @RequestBody EquipoRequestDTO dto) {
